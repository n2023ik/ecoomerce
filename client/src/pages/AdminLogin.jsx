...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-gray-700 via-gray-800 to-black p-6 overflow-hidden">
        ...
        <Card className="overflow-hidden shadow-2xl border-0 rounded-2xl bg-white/90 backdrop-blur-xl">
          <div className="bg-white p-8 rounded-2xl">
            <AnimatePresence mode="wait">
              <motion.div key="admin-login" variants={slideVariants} initial="hidden" animate="visible" exit="exit">
                <h1 className="text-3xl font-extrabold text-center text-gray-900 mb-2">Admin Sign in</h1>
                <p className="text-center text-gray-500 mb-6">Authorized personnel only</p>
                ...
                <form onSubmit={onSubmit} className="space-y-5">
                  ...
                  <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                    <Button disabled={loading} className="w-full bg-gray-900 hover:bg-black text-white text-lg py-2 rounded-xl shadow-md transition-all">{loading ? 'Signing in...' : 'Sign In'}</Button>
                  </motion.div>
                </form>
              </motion.div>
            </AnimatePresence>
