...
export default function SellerRegister() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      await register(username, email, password, 'seller')
        ...
        <Card className="overflow-hidden shadow-2xl border-0 rounded-2xl bg-white/90 backdrop-blur-xl">
          <div className="bg-white p-8 rounded-2xl">
            <AnimatePresence mode="wait">
              <motion.div key="seller-register" variants={slideVariants} initial="hidden" animate="visible" exit="exit">
                <h1 className="text-3xl font-extrabold text-center text-gray-800 mb-2">Create seller account</h1>
                <p className="text-center text-gray-500 mb-6">Start selling on ShopEase</p>
                ...
                <form onSubmit={onSubmit} className="space-y-5">
                  ...
                  <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                    <Button disabled={loading} className="w-full bg-purple-600 hover:bg-purple-700 text-white text-lg py-2 rounded-xl shadow-md transition-all">{loading ? 'Creating...' : 'Create Account'}</Button>
                  </motion.div>
                </form>
                <div className="text-center mt-6">
                  <p className="text-gray-500 text-sm">Already a seller? <Link to="/seller/login" className="text-purple-600 font-semibold hover:underline">Sign in</Link></p>
                </div>
              </motion.div>
            </AnimatePresence>
          </div>
